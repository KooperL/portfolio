...
import { ClippingVecs, Colours, Dims, mat4x4, Noise3D, transformations, translations, triangle, vec3d } from "../utils/types";
...
export function Render() {
        ...
        for(let i=0;i<4;i++) {
          ...
          while (nNewTriangles > 0) {
            ...
            switch(i) {
              case 0:
                // @ts-ignore
                nTrisToAdd = Triangle_ClipAgainstPlane(clippingVecs.vecOrigin, clippingVecs.vecY, test, clipped[0], clipped[1]);
                break;
              case 1:
                // @ts-ignore
                nTrisToAdd = Triangle_ClipAgainstPlane(clippingVecs.vecBottom, clippingVecs.vecMY, test, clipped[0], clipped[1]);
                break;
              case 2:
                // @ts-ignore
                nTrisToAdd = Triangle_ClipAgainstPlane(clippingVecs.vecOrigin, clippingVecs.vecX, test, clipped[0], clipped[1]);
                break;
              case 3:
                // @ts-ignore
                nTrisToAdd = Triangle_ClipAgainstPlane(clippingVecs.vecRight, clippingVecs.vecMX, test, clipped[0], clipped[1]);
                break;
            }
  ...
  function controlCamera(e: string) {
    ...
    switch(e) {
      ...
      case 'ArrowDown':
        ...
        break;
      case 'ArrowLeft':
        setCameraTransformations({...cameraTransformations, yaw: cameraTransformations.yaw - 0.01});
        break;
      case 'ArrowRight':
        setCameraTransformations({...cameraTransformations, yaw: cameraTransformations.yaw + 0.01});
        break;
    ...
    return new Promise(r => {
      ...
      for (let i=0;i<objects.length;i++) {
        ...
        obj.tris.forEach((tri: any) => {
          // console.log(tri)
          let triRotated = cardinalTransformation(tri, worldTransformations.roll, worldTransformations.pitch, worldTransformations.yaw);
          let triTranslated = cardinalTranslate(triRotated, worldTranslations.x, worldTranslations.y, worldTranslations.z);
          ...
          if(vec_dot_product(normal, vCameraRay) < 0) {
            ...
            const dp = (vec_dot_product(vec_normalise(lightDirection), normal)+1) / 2;
            ...
            nClippedTriangles = Triangle_ClipAgainstPlane(clippingVecs.nearPlane, clippingVecs.farPlane, triViewed, clipped[0], clipped[1]);
            if(nClippedTriangles[0] && nClippedTriangles[1]) {
              for(let i=0;i<nClippedTriangles[0];i++) {

              // Project triangles from 3d -> 2d
              let triProjected = triangleMatMul(nClippedTriangles[1][i], matproj);
  ...
  return (
    <div className="pt-20">
      <RasterSVG name='name' tris={trisToRaster} dims={dims} />
    </div>
  )
}

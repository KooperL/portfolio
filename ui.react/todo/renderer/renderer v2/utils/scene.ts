    ...
    for (let i = 0; i < this.objects.length; i++) {
      ...
      obj.tris.forEach((tri: any) => {
        ...
        let triRotated = cardinalTransformation(
          tri,
          this.worldTransformations.roll,
          this.worldTransformations.pitch,
          this.worldTransformations.yaw,
        )
        let triTranslated = cardinalTranslate(
          triRotated,
          this.worldTranslations.x,
          this.worldTranslations.y,
          this.worldTranslations.z,
        )
        ...
        if (vec_dot_product(normal, vCameraRay) < 0) {
          ...
          const dp =
            (vec_dot_product(vec_normalise(this.lightDirection), normal) + 1) /
            2

...
function DrawSVG(props: { passed: RenderProps }) {
  ...
  const move = {
    transformations: props.passed.transformations ?? {
      roll: 0.4,
      pitch: 4.7,
      yaw: 0.4,
    },
  ...
  useEffect(() => {
    const dims: Dims = {
      w: props.passed.dims.w,
      h: props.passed.dims.h,
      fov: props.passed.dims.fov,
    }
  ...
  useEffect(() => {
    if (obj) {
      obj.simpleRaster(move.transformations, move.translations)

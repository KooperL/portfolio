...
export class Obj {
  ...
  parseObj(text: string) {
    ...
    lines.forEach((item: string, ind:number) => {
      ...
      if(coords[0] === 'v') {
        this.verts.push(new vec3d(parseFloat(coords[1]), parseFloat(coords[2]), parseFloat(coords[3])));
      } else if(coords[0] === 'f') {
        let temp = new triangle()
        temp.p = [this.verts[+coords[1]-1], this.verts[+coords[2]-1], this.verts[+coords[3]-1]]
        this.tris.push(temp);
      }
    });
  }
    ...
    this.tris.forEach((tri) => {
      ...
      if(vec_dot_product(normal, vCameraRay) < 0) {
        ...
        if(nClippedTriangles[0] && nClippedTriangles[1]) {
          ...
          let triProjected = triangleMatMul(nClippedTriangles[1][i], matProj(this.dims.w, this.dims.h, this.dims.fov));

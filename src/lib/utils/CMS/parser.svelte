<script lang="ts">
    ...
    // import EmbeddedFrame from '$lib/components/EmbeddedFrame.svelte';
    import type { Content, ContentElement, TextBody, Card, Form, EmbeddedFrame } from '$lib/utils/CMS/types';
    ...
    // Content type guards
    const isTextBody = (element: ContentElement): element is { type: 'textBody'; content: TextBody } =>
      element.type === 'textBody';
      
    const isCard = (element: ContentElement): element is { type: 'card'; content: Card } =>
      element.type === 'card';
      
    const isForm = (element: ContentElement): element is { type: 'form'; content: Form } =>
      element.type === 'form';
      
    const isEmbeddedFrame = (element: ContentElement): element is { type: 'embeddedFrame'; content: EmbeddedFrame } =>
    ...
    function validateContent(content: Content): boolean {
      try {
        ...
        if (!content.pageContent || !Array.isArray(content.pageContent.elements)) {
        ...
        content.pageContent.elements.forEach(element => {
          if (!element.type || !element.content) {
            throw new Error(`Invalid element structure: ${JSON.stringify(element)}`);
          }
        });
      ...
      {#each sortElements(parsedContent.pageContent.elements) as element}
        <section class="mb-8">
          ...
          {:else if isCard(element)}
            <CardComponent {...element.content} buttonActions={functions}/>
          {:else if isForm(element)}
            <FormComponent {...element.content} />
          <!-- {:else if isEmbeddedFrame(element)}
            <EmbeddedFrame {...element.content} /> -->
          {:else}
            <Alert color="yellow">

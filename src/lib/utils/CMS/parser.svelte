...
{#if node}
  ...
  {#if node.type === "title"}
    <h2
      class="mb-4 text-3xl font-extrabold text-gray-700 dark:text-white {node.styling ??
        ''}"
    >
      {node.props.value ?? ""}
    </h2>
  {:else if node.type === "text"}
    <div
      class="py-2 px-2 mx-auto max-w-screen-xl lg:px-12 text-left {node.styling ??
        ''}"
    >
      <div class="space-y-4 text-gray-700 dark:text-gray-300">
        {#each (node.props.value ?? "")
          .split("\n\n")
  ...
  {:else if node.type === "hero"}
    <div
      class="relative bg-cover bg-center pt-8 pb-24 px-16 sm:py-16 sm:px-4 {node.styling ??
        ''}"
    >
      <div class="background bottom bg-primary-700"></div>
      <div class="background top bg-gray-300"></div>
      <div
        class="relative z-3 max-w-screen-xl mx-auto text-center"
        style="z-index:3;"
      >
        <h1
          class="font-extrabold tracking-tight text-gray-700 leading-none text-4xl md:text-5xl lg:text-6xl"
        >
          {node.props.title ?? ""}
        </h1>
        <p
          class="mb-8 font-normal text-gray-700 text-lg lg:text-xl sm:px-16 lg:px-48"
        >
          {node.props.subtitle ?? ""}
        </p>
        <div class="my-16"></div>
  ...
  {:else if node.type === "card"}
    <Card class="w-full max-w-full">
      <div
        class={node?.props?.["horizontal-layout"] === "true"
          ? "flex flex-row"
          : ""}
      >
        {#if node.props.image}
          <div class="flex justify-center items-center max-h-max h-max">
            <img
              src={node.props.image}
              alt={node.props.title ?? node.id}
              class="rounded-t-lg overflow-hidden w-96 h-48"
            />
          </div>
        {/if}
        <div class="flex flex-col">
          <div class="p-4 flex flex-col">
            ...
            {#if node.props.description}
              <p
                class="mb-3 font-normal text-gray-700 dark:text-gray-400 text-wrap"
              >
                {node.props.description}
              </p>
            {/if}
            {#if node.props.url}
              <div class="mt-2">
                <Button
                  ...
                  href={node.props.url.startsWith("/")
                    ? `${base}${node.props.url}`
                    : node.props.url}
                  variant="primary"
                >
  ...
  {:else if node.type === "button"}
    <Button
      class="custom-button rounded text-nowrap max-w-max"
      style="-webkit-clip-path: polygon(0 0, 100% 0, 100% 66%, 78% 100%, 0 100%); clip-path: polygon(0 0, 100% 0, 100% 66%, 78% 100%, 0 100%);"
      href={node.props.url
        ? node.props.url.startsWith("/")
          ? `${base}${node.props.url}`
          : node.props.url
        : undefined}
      variant="primary"
  ...
  {:else if node.type === "embeddedFrame"}
    <iframe
      src={node.props.url ?? ""}
      class="w-full h-128 {node.styling ?? ''}"
    />
